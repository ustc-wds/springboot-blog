...
import com.hyf.bean.Comment;
import com.hyf.bean.UserReadNews;
import com.hyf.mapper.CommentMapper;
import com.hyf.redis.HashRedisUtil;
import com.hyf.service.ArticleService;
import com.hyf.service.CommentLikesRecordService;
...
public class CommentServiceImpl implements CommentService {
    ...
    private void addNotReadNews(Comment comment){
        if(comment.getRespondentId() != comment.getAnswererId()){
            boolean isExistKey = hashRedisUtil.hasKey(comment.getRespondentId()+ StringUtil.BLANK);
            if(!isExistKey){
                UserReadNews news = new UserReadNews(1,1,0);
                hashRedisUtil.put(String.valueOf(comment.getRespondentId()), news, UserReadNews.class);
            } else {
                hashRedisUtil.hashIncrement(comment.getRespondentId()+ StringUtil.BLANK, "allNewsNum",1);
                hashRedisUtil.hashIncrement(comment.getRespondentId()+ StringUtil.BLANK, "commentNum",1);
            }
        }
    }
