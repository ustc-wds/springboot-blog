...
var email_pattern = /[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/;
...
loginFormSubmit.click(function () {
    ...
    console.log(isEmail);
    if(password_len !== 0 && email_len !== 0 && email_pattern.test(email_value)){
        $.ajax({
            type:'post',
            url:'/login',
            dataType:'json',
...
modal_email.blur(function () {
    ...
    if(modal_email_len !== 0){
        if(!email_pattern.test(modal_email_value)){
